...
- 日期统一转换为YYYY-MM-DD
- 无法识别的字段返回null，不要猜测
- **括号规范**：如果buyer_name或seller_name包含中文，则名称中的括号必须使用全角符号（）而非半角符号()，例如"北京（上海）科技有限公司"不能写成"北京(上海)科技有限公司"

请直接返回JSON对象："""

# Text-based extraction prompt (OCR text → LLM structured output)
INVOICE_TEXT_SYSTEM_PROMPT = """你是一个专业的中国发票信息提取助手。
你必须严格按照JSON Schema格式返回结果，无法识别的字段返回null。
只返回JSON对象，不要包含任何其他文字、解释或markdown代码块标记。"""

INVOICE_TEXT_PROMPT = f"""请从以下OCR识别出的发票文本中提取结构化信息。
